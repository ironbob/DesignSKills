...
9. **写 `outline.json` → 跑 `validate_outline.py`** —— 按 `references/outline-schema.md` 把提纲结构化为**机器契约源**（交接物），写到 `book_talk/YYYY-MM-DD-<书名-slug>-outline.json`，跑 `scripts/validate_outline.py`，通过才进下一步。
