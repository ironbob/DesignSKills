...
2. **规范即事实源（完整契约 + 严格投影）** —— 先产出结构规范（EPPS 页面对象 + 跳转图 + `sample_state`）；HTML 是规范的**机械投影**——每个内容区对回一条 `density.zones[]` 声明，每个辅助元素对回一条 `assistive_elements[]` 声明，每个跳转对回 `jump`，不脱离规范凭空画。逐页「定稿 EPPS → 立刻草渲」保持连贯，完整 22 条校验通过后才组装交付。
...
<HARD-GATE>
完整 22 条校验通过（所有 🔴 ERROR 清零、🟡 WARNING 通过率 ≥ 80%）、HTML/spec 对账通过、**且需求→页面规划校验通过（`page_plan` 齐全：每个 P0 已交付或合法 cross_cutting、变体平铺渲染）**之前，**不组装最终 `prototype.html`、不交付**。
...
在开始拆页面之前，必须询问用户是否要参考当前项目中的其他文件、代码、历史原型、旧需求文档、路由、组件或 README。不要先搜索/读取项目内容再决定是否采用；项目里偶然发现的内容不能自动当作当前需求事实源，因为已有代码/文档可能过时。

**默认规则**：
- 用户只给一句需求或一个需求文档时，只以该输入为事实源；不主动扫描项目其他目录寻找补充需求。
- 不为了"找参考源"先运行 `rg` / `find` / 目录遍历；只有用户允许参考并指定范围后，才读取对应路径。
- 启动时先问一句："是否需要参考已有项目文件或代码？如果要，请指定路径，并说明是当前需求事实源还是仅供上下文。"
- 用户说不参考时，设置 `prototype.project_references.mode: none`，`items: []`。
- 用户明确给出路径时，只读取该路径；仍需判断它是当前事实源、仅供上下文，还是可能过时。
- 现有代码与用户当前描述冲突时，以用户当前确认的需求为准，把冲突写入"未决问题"，不要静默沿用旧实现。
- 所有参考源写入 `prototype.project_references.items`：路径、类型、角色、时效性、采用/忽略理由。

**参考源角色**：
- `source_of_truth`：用户明确确认的当前需求事实源。
- `context_only`：只用于理解现状、命名、已有入口，不得新增/覆盖需求。
- `stale_or_conflicting`：疑似过时或与当前需求冲突，只记录问题，不作为设计依据。
...
在拆页面、画跳转、声明 `sample_state` 之前，先判定本次原型是**整 App** 还是**App 内某个功能流程**。不得默认画整 App。

**判定为 `whole_app`**：
- 用户明确说"整个 App"、"完整应用"、"从 0 到 1"、"主结构"、"首页/课程/我的都要"、"主导航/底部 Tab"。
- 需求目标是定义产品主信息架构，而不是某个入口到完成的单段流程。
- 产出 level1 页面，通常包含 `home` / `profile` / 主要 `list`，`tab_bar_mode: inherit`，底部 Tab 3–5 个。

**判定为 `feature_flow`**：
- 用户明确说"某个功能"、"流程"、"模块"、"页面"、"从入口到完成"、"错题重做"、"课程报名"、"购买/提交/设置某项"。
- 需求可被描述为宿主 App 中一条任务流，而不是产品主结构。
- 默认 `tab_bar_mode: hidden`；不派生 `home` / `profile` 这类 level1 壳页面；必须声明 `host_anchors` 表示入口/出口。

**不明确时，停止继续设计，只问一句**：

> 这是要设计整个 App 的主结构，还是 App 中某个功能流程？
...
9. **自审（含机械化对账 + 需求→页面规划）** —— placeholder 扫描、孤立页、死胡同、密度复核；运行 `scripts/audit_html_projection.py prototype.md prototype.html` 做**机械化对账**（HTML↔spec：zone 数量/顺序/`kind` 一一对应、示例数据同源 `sample_state`、affordance 单点 `placement`、无未声明跳转），硬拦截不一致，就地修；**再运行 `scripts/validate_page_plan.py prototype.md <需求.md> prototype.html` 做「需求→页面规划」第三道门**（`page_plan` 结构/plan↔pages 对等/平铺渲染齐全、每个 P0 已交付或合法 cross_cutting、`delivers` 无悬空），P0 未交付或变体未平铺渲染即阻断，就地补 `page_plan`/补页面/修塌缩。详见 `references/html-render-template.md` §五 与 `references/validation-rules.md`「第三道门」。
...
digraph prototype {
  ...
  "自审(含机械化对账)" -> "需求→页面规划(P0全覆盖+平铺)?";
  "需求→页面规划(P0全覆盖+平铺)?" -> "自审(含机械化对账)" [label="否,补page_plan/修塌缩"];
  "需求→页面规划(P0全覆盖+平铺)?" -> "交付" [label="是"];
}
```
...
- **规范即事实源（完整契约 + 严格投影）** —— HTML 是 spec 的**机械投影**：每个内容区对回一条 `density.zones[]` 声明（`zone.kind` 取闭环枚举），每个辅助元素对回一条 `assistive_elements[]` 声明，每个可点元素对回 `jump`/`primary`/`back`/`tab`。**不渲染 spec 未声明的 zone/assistive element，也不出现 spec 没有的跳转**。
- **元素先声明意图再承载** —— 每个可见元素必须有 `element_contract`；`guidance` 禁止 `main_content`，默认进入 `coachmark`、`bottom_sheet`、`inline` 或 `modal`。
...
| 偷偷扫描项目旧代码并按旧实现画原型 | 先问是否参考；旧代码默认 `context_only`，不能覆盖当前需求 |
| 现有文件与用户新需求冲突时沿用现有文件 | 以用户确认需求为准，冲突写入未决问题 |
...
| scope 不明确还继续拆页面 | 只问一句"整 App 还是功能流程？"，确认后再继续 |
| 渲染时凭空加 zone（如把「学习提示」画成主内容区） | 判断意图：核心内容块进 `density.zones`；引导/帮助/提示进 `assistive_elements`，渲染只投影声明值 |
| 把学习引导/帮助说明放进主内容区 | 设为 `assistive_elements`，`intent: guidance`，surface 用 `coachmark` / `bottom_sheet` / `inline` / `modal` |
| 元素缺少意图/承载字段 | 所有 zones/actions/assistive elements 必填 `element_contract`，缺失即校验失败 |
...
- **`references/page-plan-judge.md`** —— LLM 裁判提示词（**建议性、不阻断**）：复核 `page_plan` 的塌缩/过度拆分/cross_cutting 误分类。**起草 page_plan 后运行**。
- **`scripts/validate_epps.py`** —— 校验 EPPS schema 与 22 条规则。**最终交付前必须运行**。
- **`scripts/audit_html_projection.py`** —— 校验 HTML 是否严格投影 EPPS。**最终交付前必须运行**。
- **`scripts/extract_requirements.py`** —— 从需求 md 抽取原子需求项（带确定性 id `REQ-M##-##`），是需求覆盖契约的来源。**规划页面时运行**。
- **`scripts/validate_page_plan.py`** —— 第三道门：校验 `page_plan`（结构/对等/平铺渲染/每 P0 已交付或合法 cross_cutting）。**最终交付前必须运行**。
