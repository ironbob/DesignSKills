...
2. **定位 + 建代码地图 + 拟应用/领域画像（只读不深挖）** —— 从自然语言定位业务入口，建代码地图（关键文件 + 入口 `file:line` + 职责）；读仓库信号拟应用画像 / 整体架构 / 领域定位；**行业惯例锚点按获取链取**（用户文档 > 模型推断明牌待确认），frontmatter 记 `domain_source`。详见 `references/locating-business.md` + `references/domain-profiling.md`。
