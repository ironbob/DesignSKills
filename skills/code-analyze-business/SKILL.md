...
**产出①（现状层 analysis.md）**：
- 应用与领域定位（应用画像 / 整体架构 / 领域定位 / 行业惯例锚点）、业务概述、触发与入口、核心领域概念、主流程（Mermaid + 回链 + **每步异常/兜底/兼容**）、**异常·兜底·兼容专项矩阵**、数据与存储、依赖与耦合、风险与技术债、代码地图（详见 `references/analysis-template.md`）

**产出②（反推需求 requirements.md，PM 视角）**：
- 一句话目标、目标用户（主要/次要/**NOT**）、核心场景
- 功能清单 ★（模块表：优先级 P0/P1/P2 + 功能 + 简述 + **验收标准（业务语言，可判定）** + **实现状态** ✅/⚠️/❌ + **实现锚点**）
- 明确不做、非功能约束
- **实现与需求偏差** ★（反推独有：代码做了但需求未必需要的 / 需求该有但代码缺失的）
- 已知缺口（详见 `references/requirements-template.md`）

**产出③（测试用例 test-cases.md）**：
- 按 Module 组织，每个用例 `TC-<MODULE>-<n>` / Type（Happy / Error / Edge）/ Preconditions / Steps / **Expected Result（业务可观察断言）** + **实现锚点（file:line）** / 需求来源
...
2. **定位 + 建代码地图 + 拟应用/领域画像（只读不深挖）** —— 从自然语言定位业务入口，建代码地图（关键文件 + 入口 `file:line` + 职责）；同时读仓库级信号（README / manifest / 目录 / 依赖 / migration）拟「应用画像 + 整体架构 + 领域定位 + 行业惯例锚点」。详见 `references/locating-business.md` + `references/domain-profiling.md`。
3. **两段式确认（HARD-GATE）** —— 把应用画像 + 领域定位 + 代码地图 + 一句话业务范围（触发词 X / 入口 Y / 边界含 A 不含 B）呈现给用户确认。未确认不进下一步。最大风险是"找错业务 / 领域判断偏"，这一步专门拦它。
4. **深挖主流程 + 领域概念** —— 在确认后的边界内，追踪主流程调用链、识别核心领域概念与状态机；**主流程每步讲透决策 + 适用项的异常/兜底/兼容**，不能只写"调了谁"。详见 `references/tracing-flow.md`。
...
8. **反推需求 requirements.md** —— analysis 通过后，按 `references/reverse-prd.md` 把现状翻译成 PM 视角需求（对齐 `references/requirements-template.md`：功能清单每行 **验收标准用业务语言、代码归「实现锚点」列** + 实现状态 ✅/⚠️/❌；写「实现与需求偏差」节；借领域定位 / 行业惯例判断"应有"）。写到 `docs/business-analysis/YYYY-MM-DD-<业务名>-requirements.md`，跑 `validate_requirements.py`，通过才进下一步。
9. **派生测试用例 test-cases.md** —— 按 `references/test-case-generation.md`，从 requirements + analysis 生成覆盖所有需求与边缘 case 的用例（对齐 `references/test-cases-template.md`：**Expected Result 用业务可观察断言 + 独立「实现锚点」行**）。写到 `docs/business-analysis/YYYY-MM-DD-<业务名>-test-cases.md`，跑 `validate_test_cases.py`。
...
digraph analyze {
  ...
  "业务范围确认?" -> "深挖主流程+概念(异常/兜底/兼容)" [label="是"];
  "深挖主流程+概念(异常/兜底/兼容)" -> "补异常兜底兼容矩阵+数据/依赖/风险";
  ...
  "validate_requirements?" -> "派生test-cases(黑盒断言)" [label="是"];
...
- **领域先行** —— 先拟应用画像 + 领域定位 + 行业惯例锚点并让用户确认，再深挖；反推需求与用例都借这套 framing 讲业务，不贴着代码功能列。
- **两段式确认** —— 先建地图 + 拟领域画像 + 陈述范围请用户确认，再深挖；最大失败模式是找错业务或领域判断偏。
- **读码取证** —— 每条结论回链 `file:line`；无依据标 `⚠ 未确认`，不臆造。
- **业务断言 vs 实现锚点分层** —— requirements 验收标准 / test-cases 的 Expected Result 用**业务可观测语言**（PM/测试黑盒可验），代码事实（函数/路由/表名/枚举/状态码）一律归「实现锚点」列/行，不写进断言。analysis 层相反——代码就是主角。
- **主流程讲透决策与兜底** —— 主流程每步写做什么 + 适用项的异常/兜底/兼容，不只写"调了谁"；散落的收进 §5 异常·兜底·兼容矩阵。
...
**analysis（现状层）**
- **`references/domain-profiling.md`** —— 怎么读仓库信号拟应用画像 + 整体架构 + 领域定位 + 行业惯例锚点，Checklist 第 2 步用
