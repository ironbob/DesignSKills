...
> 配合 `code-analyze-business` 阶段 2 使用。前提：业务范围已确认。这一步解决“该找的都找全了吗”——`tracing-flow.md` 教你怎么把一条主链路追透，本文件教你怎么保证没有遗漏的入口、旁路、数据副作用与横切逻辑。
...
3. **不可用时自动降级**：不安装工具、不暂停等待；立即用 `rg`（不可用则用等价文本搜索）搜函数名 / 类名 / 路由 / topic / 状态枚举 / 表字段，并人工确认调用关系。在进度里简短告知用户，在「链路覆盖」节标注「LSP unavailable，符号覆盖为文本搜索降级」。

4. **仅歧义时求助**：文本搜索结果不足以区分多个实现、动态注册或反射调用，且会实质影响业务边界时，才向用户询问或建议补装对应 language server。安装不是继续工作的前置条件。

> 原则：优先精确符号查询，但工具缺失不能制造额外流程门禁。
...
> 无 LSP 时把每一行换成“文本搜索符号名 → 人工判定调用关系”，并在节首标注降级。
