...
### 工作区 chrome（materials.md 配方）

| 类 | 规格 |
| --- | --- |
| 底部工具条 | 画布底部水平居中悬浮，高 36-44、r12，内含高频工具；全局分享等业务动作不必塞入工具条 |
| 图层面板 | 左侧停靠、宽 240-260 且可调/可折叠，顶部 tabs + 图层树 + 底部计数行 |
| 属性面板 | 右侧停靠、宽 240-280 且可调/可折叠，顶部 tabs + 分节属性区 |
| 临时面板 | 仅 Minimize UI/上下文模式浮动，使用 r12 + panel shadow |
