...
> 身份特征：**中灰画板 + 可折叠停靠面板 + 底部 slim toolbar + 全目录最微型控件 + 选中蓝画布语义**。
...
| `--fig-panel` | `#ffffff` | `#2c2c2c` | 停靠面板与底部工具条；只有工具条/临时面板配大柔投影 |
...
| | 亮 | 暗 |
| --- | --- | --- |
| 底部工具条/临时面板 | `0 12px 32px rgba(0,0,0,.12), 0 2px 8px rgba(0,0,0,.06)` | `0 12px 32px rgba(0,0,0,.5), 0 2px 8px rgba(0,0,0,.35)` |
| 浮层小投影 | `0 4px 12px rgba(0,0,0,.18), 0 1px 3px rgba(0,0,0,.1)` | `0 4px 12px rgba(0,0,0,.55), 0 1px 3px rgba(0,0,0,.4)` |

停靠面板不使用常驻大阴影。无 backdrop blur——chrome 是实色，不是毛玻璃。
