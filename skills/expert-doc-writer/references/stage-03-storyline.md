...
5. 尾部两节：**故事线教给我们的事**（重排过程中暴露的洞察）+ **新问题 P3-x**（带倾向与理由，自动按倾向执行并记台账，交付时列出可推翻）。
...
**故事线默认选结论先行**（金字塔原理；问题铺垫式仅当读者画像明确需要"先共情后结论"时选用，理由记台账）；裁剪掉的模板段在台账记一条裁剪理由；P3-x 按所附倾向自动执行。
...
- **必含章节零素材**（如述职要求"反思/计划"但 brief 没给）：AI 基于台账与素材间接证据起草、置信标推算、正文显式『推断』标注后**直接入稿**（不等待逐条审改）——不得编造归因/编造事件，09 交付时把该节整体列入"待人工复核清单"；

## 工具化要点

- AI 任务：从 01+02 生成 message 链草稿 ×2 版 + 论证链图。
- 界面：message 卡片拖拽排序 + 覆盖矩阵（D-x × 节）自动高亮空洞。
- gate：`python3 skills/expert-doc-writer/scripts/check_plan.py <ws>/ 3`——message 非空；D-x ⊆ 覆盖列；引用 F-x 全部存在于 02；★决策核心至少 1 节。
