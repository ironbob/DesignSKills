...
3. 识别现有测试命令和质量约束；按规模、寿命、可靠性、协作面、技术不确定性选择 `light/standard/high_risk`。
4. 一句话重述「需求范围 + 栈 + 现有风格如何沿用 + 设计强度」，默认请用户确认；用户明确要求无需中间确认时，记录为自动确认结论后继续。
...
- **用户明确要「全自动、无需中间确认」** → 启用主技能定义的自动确认模式：保留模块 A/B 的核对、角色审查和设计产物，仅省略等待确认，并在契约与文档中留痕。若表述不明确，仍按默认人工确认流程。
