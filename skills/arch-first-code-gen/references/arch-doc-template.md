...
渲染 `interaction_confirmation`，至少写明：用户选择的等级、`proposal_revision`、确认的候选方案、确认来自方案展示后的后续用户消息。证据应准确概述消息，不伪造逐字引文。
...
- 等级选择：用户选择 `standard`。
- 当前方案版本：`1`。
- 方案确认：用户在完整方案展示后的后续消息中确认 `ALT-1` 进入编码。
```

## 四、模块结构图（mermaid — R-S 校验「结构图」）
