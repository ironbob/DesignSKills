...
Gate 2 必须传入 `assets-manifest.json` 和真实 `code-root`。所有 delivered 锚点的文件和
...
| `DLV.architecture` | ERROR | 已执行 arch-first，记录调用/确认方式、设计契约、架构文档和验证证据 |
| `DLV.architecture.file` | ERROR | 设计契约与架构文档真实存在且不越出 code-root |
...
`DLV.architecture` 是先于上述视觉交付的架构硬门：pic-to-ui 不能自行编码后再补一条“符合架构”的声明。其字段和交接顺序见 `architecture-handoff.md`。
