...
- **`references/domain-analysis.md`** —— 领域分析怎么做：分析维度、如何呈现、何时需联网，**领域知识树（`domains/`）的定位 + 逐层回退**，以及**拳头产品兜底（模型内置知识 + 来源标注）**
- **`references/拳头产品速查.md`** —— 各领域拳头产品速查 + 覆盖地图（内置命中优先；未命中走模型内置知识兜底）
