...
<HARD-GATE>
工具包**自校验通过**（`scripts/validate_toolkit.py`：内容列表/schema/生成脚本/校验门四者自洽）、**样本验证通过**（`scripts/run_sample_validation.py`：样本端到端跑通、过核心质量门、中断/恢复/重试/幂等/多文件确实工作）之前，**不交付工具包**。
skill **不替用户跑全量数据**；交付边界止于"工具包 + 样本验证"。
</HARD-GATE>

## 反模式：直接开始造数据

不要跳过"分析需求+页面→内容列表+schema→工具包"直接用 LLM 造一批数据交付。skill 的价值是**工具包**（可重跑、可续跑、带门），不是一次性数据。一次性数据交付属于越界（见边界）。
...
3. **产出内容列表 + schema** —— 产出 `content_list.json`（内容点清单：id/类型/`年级×Bloom`/来源/种子数据）+ 实体 schema。无骨架时，内容列表含"先生成大纲/知识点"的内容点。格式见 `references/toolkit-structure.md`。
...
5. **工具包自校验 + 确认门** —— 运行 `scripts/validate_toolkit.py <toolkit_dir>`：内容列表/schema/生成脚本/校验门四者自洽（字段对齐、脚本可导入、门配置合法、无占位）。不通过就地修。**通过后请用户确认工具包**，确认才进样本验证。
...
digraph edudatagen {
  ...
  "综合数据需求推断" -> "产出内容列表+schema";
  "产出内容列表+schema" -> "搭工具包(复制模板+填config)";
  "搭工具包(复制模板+填config)" -> "工具包自校验";
  "工具包自校验" -> "自洽?";
  "自洽?" -> "产出内容列表+schema" [label="否,修"];
  "自洽?" -> "用户确认工具包" [label="是"];
...
- `content_list.json` —— 内容点清单
- `schema/` —— 实体 schema（每类一个文件或一个 schemas.json）
- `generate.py` —— 生成脚本（中断/恢复/重试/幂等/多文件）
- `validate.py` —— 校验门运行器
- `config.json` —— 产品配置
- `prompts/` —— prompt 模板（每内容类型一个）
- `state/` —— resume 状态目录（运行时生成）
- `output/` —— 生成数据（样本验证时产出样本子集）
