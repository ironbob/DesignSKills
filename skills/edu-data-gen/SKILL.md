...
- **内置 语数英科学 完整课纲+知识点库**（`curriculum_full/` + `curriculum/` 兼容索引）：作为课纲参考的可选来源（P1 课纲对齐能力包）。

## Checklist

为以下每项创建一个 task，按序完成：

1. **加载输入 + 判定模式 + 课纲来源** —— 读取需求文档 + 页面文档；判定有无【骨架】→ "骨架补全"或"从零生成"；用一句话重述"该产品要生成哪些数据、给谁用"，请用户确认。若启用课纲对齐：**询问**课纲参考来源——用户提供 / 用内置 语数英科学 完整库 / 跳过（非语数英科学 只能提供或跳过）。加载 `references/data-types-and-schemas.md`。
...
- **课纲对齐可选** —— 启用时询问来源（用户/内置完整库/跳过）；非语数英科学 需用户提供或跳过。
...
- **`references/curriculum_full/`** —— 内置 语数英科学 结构化完整课纲+知识点库（g1-g9 JSON）。**启用课纲对齐且选内置库时优先加载**。
- **`references/curriculum/`** —— 由完整库生成的 Markdown 兼容索引（README + 语文/数学/英语/科学）。**旧式工具包或人工快速浏览时加载**。
